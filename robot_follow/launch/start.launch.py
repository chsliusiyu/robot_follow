...
def generate_launch_description():
    pkg_share = get_package_share_directory('robot_follow')
    ...
    enable_kalman_arg = DeclareLaunchArgument(
        'enable_kalman',
        default_value='false',
        description='是否启用enable_kalman'
    )
    
    # 创建节点
    robot_nexus_node = Node(
        package='robot_follow',
    ...
    robot_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(get_package_share_directory("agibot"), 'launch', 'agibot.launch.py')
        )
    )
    ...
    uwb_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(get_package_share_directory("uwb_serial_pub"), 'launch', 'uwb_serial_pub.launch.py')
        )
    )
